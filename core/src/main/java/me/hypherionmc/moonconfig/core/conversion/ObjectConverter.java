...
public final class ObjectConverter {
			...
			for (Field field : clazz.getDeclaredFields()) {
				...
				if (value == null) {
                    ...
                    if (Enum.class.isAssignableFrom(valueType)) {
						...
						applyComment(field, destination);
	...
	/***
	 * Checks if @SpecComment is present on a field and apply the comment to the config, if supported
	 * @param field The config field being parsed
	 * @param destination The destination config to apply the comment to
	 */
	private void applyComment(Field field, Config destination) {
